...
export const geoURL = (location) => `${GEO_BASE_URL}q=${location}&limit=${limit}&appid=${APP_ID}`;

export const airQualityURL = (lat, lon) => `${AP_BASE_URL}lat=${lat}&lon=${lon}&appid=${APP_ID}`;

export const forcastAqURL = (lat, lon) => `${FORCAST_AP_BASE_URL}lat=${lat}&lon=${lon}&appid=${APP_ID}`;
...
export const handleRatings = (data = aqiRatings, aqi) => data.filter((ratings) => ratings.aqi === aqi);

export const handleColorMap = (dataArr) => dataArr.map((aqi) => handleRatings(aqiRatings, aqi)[0].color);

export const handleBorderColorMap = (dataArr) => dataArr.map((aqi) => handleRatings(aqiRatings, aqi)[0].borderColor);
